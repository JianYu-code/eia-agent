from app.knowledge.retriever import STD_ID_RE, search_knowledge
...
async def ask_knowledge(question: str, history: list[dict] = None) -> tuple[str, list[dict]]:
    ...
    queries = [question]
    if profile and not STD_ID_RE.search(question):
        try:
            expand = await chat(
                "将以下环保标准检索问题改写为2个更精确的检索查询（尽量含相关标准编号或标准全称），"
                f"每行一个查询，不要输出其他内容：{question}",
                profile=profile)
            queries += [l.strip().lstrip("0123456789.、 ") for l in expand.splitlines()
                        if l.strip() and len(l.strip()) >= 4][:2]
        except Exception:
            pass
    ...
    results = []
    for q in queries[:3]:
        for r in search_knowledge(q, top_k=8):
    ...
    for i, r in enumerate(reranked):
        ...
        dep_tag = f" [已废止，替代: {r['replaced_by']}]" if r.get("deprecated") and r.get("replaced_by") else (" [已废止]" if r.get("deprecated") else " [现行]")
        context_parts.append(
            f"【来源{i+1}】{r['title']}{std_tag}{dep_tag}\n"
            f"章节: {r.get('category', '')} / {r.get('relative_path')}\n"
            f"正文:\n{r['excerpt']}"
        )
    ...
    prompt = f"""以下是标准知识库检索到的相关标准原文（按相关性和现行有效性排序，每条已标注现行/废止状态）：
...
请严格基于上述检索结果回答，并遵守：
- 每条依据必须注明其标准的现行/废止状态；若用户询问现行版本，优先采用现行标准并提示替代关系
- 如果检索结果不足以支撑回答，请说明"当前知识库未收录"，不要编造。"""
    ...
    sources = [
        {
            ...
            "source": r.get("source", ""),
            "vault_path": r.get("source", "").removeprefix("vault/") if r.get("source", "").startswith("vault/") else "",
